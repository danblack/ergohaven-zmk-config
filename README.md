...
**без привязки к таймингам**):

- **тап** (нажал-отпустил без других клавиш, любой длительности) → `&to 13`:
  полное переключение на `MUS`, слой остаётся активным (возврат — `&TO_EN`
  внутри `MUS`);
- **с другой клавишей** (нажал `tomo`, затем клавишу мыши/скролла) → `&mo 13`:
  momentary-режим — пока клавиша зажата, мышь/скролл работают, при отпускании
  возврат на исходный слой. Удобно «быстро проскроллить», не переключаясь
  полностью.

Поведение `tomo` — `zmk,behavior-hold-tap` с `bindings = <&mo>, <&to>`,
`flavor = "hold-preferred"` и заведомо большим `tapping-term-ms` (60000), чтобы
hold/tap решался не временем, а фактом нажатия другой клавиши (см. блок
`behaviors` в `config/op36.keymap`).

По сути `tomo` — это аналог штатного `&lt 3 SPACE` (тап → действие, удержание →
слой), только на тапе вместо `&kp` стоит `&to` (toggle слоя, чего `&lt` не
умеет):

| клавиша | hold | tap |
|---|---|---|
| `&lt 3 SPACE` | `&mo 3` | `&kp SPACE` |
| `&tomo 13 13` | `&mo 13` | `&to 13` |

Разница только в `tapping-term`: у `&lt` остаётся дефолтный порог (для пробела
незаметно, его тапают быстро), а у `tomo` порог фактически снят, чтобы
осмысленный (не молниеносный) тап по клавише мыши всё равно давал переключение.
